...
trait ValidatesStrings
{
    ...
    protected function validateMinLength(mixed $data, array $schema): bool
    {
        ...
        if ($length < $minLength) {
            ...
            $this->addError('minLength', sprintf('String length %d is less than minimum length %d', $length, $minLength));
    ...
    protected function validateFormat(mixed $data, array $schema): bool
    {
        ...
        $valid = $this->performFormatValidation($data, $schema['format']);

        if (!$valid) {
            $this->addError('format', sprintf('String does not match format "%s"', $schema['format']));
    ...
    {
        // Check for custom format validator first
        if (FormatRegistry::has($formatName)) {
            $customValidator = FormatRegistry::get($formatName);

            return $customValidator?->validate($data) ?? true;
        }

        // Fall back to built-in format validators
